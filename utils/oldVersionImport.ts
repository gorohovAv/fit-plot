...
export function parseOldVersionFormat(text: string): OldVersionExercise[] {
  ...
  for (const line of lines) {
    // Check if line is an exercise name (doesn't contain 'x', 'х', numbers followed by x/X, or date patterns)
    ...
    if (isExerciseName) {
      // If we were processing a previous exercise, save it
      if (currentExercise) {
  ...
  // Don't forget to add the last exercise if it exists
  if (currentExercise) {
    exercises.push(currentExercise);
  }
